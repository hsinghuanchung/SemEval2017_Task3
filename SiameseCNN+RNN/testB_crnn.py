...
for child in root :
    	...
    	orgq_body = re.sub("[^a-zA-Z^0-9^ ]+" , '' , child[1].text) 
    	relq_subject = child[2][0][0].text
    	relq_body = re.sub("[^a-zA-Z^0-9^ ]+" , '' , child[2][0][1].text)
    	qdict.append(orgq_id)
    	cdict.append(relq_id)
    	x_query.append(orgq_body)
    	x_comment.append(relq_body)
    	"""
    	for i in range(1 , 11):
    		...
    		if rel_comment in cdict.keys():
    			rel_comment += ' '
    			x_comment[-1] += ' '
    		cdict[rel_comment] = relc_id
...
y_predict = y_predict.reshape(880)
