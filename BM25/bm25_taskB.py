...
relq_dict = {}     # original  query  id to (related query id ,related query body)  dictionary
...
def build_dict(filename):
    ...
    for child in root :
    	...
    	if orgq_id in relq_dict.keys() : relq_dict[orgq_id].append((relq_id , relq_body))
    	else :    relq_dict[orgq_id] = [(relq_id , relq_body)] 	
...
def main():
	...
	for doc_list in relq_dict.values() :
	    for doc in doc_list:
	        N += 1
...
###################   writing   output file ####################################################	
	...
	for key in qdict.keys() :
		score = []
		for  c in relq_dict[key]: 
			score.append((scoring(qdict[key][1].lower() , c[1].lower()) , c[0]))
