...
qdict = {}     #related query id to related query body dictionary
cdict = {}     # related  query  id to (related comment id ,related comment body)  dictionary
...
def build_dict(filename):               
    ...
    for child in root :
    	...
    	qdict[relq_id] = (orgq_id , orgq_subject , orgq_body , relq_subject , relq_body)
    	comment_list = []
    	for i in range(1 , 11):
...
###################   writing   output file ####################################################
	...
	for key in qdict.keys() :
		score = []
		for  c in cdict[key]: 
			score.append((scoring(qdict[key][4].lower().replace(',', '').replace('.', '').replace('?', '').replace('-', '').replace('#', '') , c[1].lower()) , c[0]))
